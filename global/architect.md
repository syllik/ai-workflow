...
- Prepared assembled execution context has a hard 32768-byte UTF-8 aggregate budget. Sol must use progressive disclosure and the minimum required context; when assembly exceeds the budget, narrow it rather than broad-loading additional files. Required context must not simply be discarded to make the number pass: split the task or block when required context cannot fit safely.
