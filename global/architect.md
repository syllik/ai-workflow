...
- When a project is added to `workspace.yaml`, create or update a coordinated `syllik/syllik` change for the `README.md` project navigation, `docs/workspace.md`, and `docs/repositories.md`. Do not treat the registry change as ready for human merge until the profile-documentation change exists and is synchronized with it, unless the human explicitly decides the project must stay omitted from profile documentation.
