...
- When a project is added to `workspace.yaml`, include synchronization of the `syllik/syllik` `README.md` project navigation, `docs/workspace.md`, and `docs/repositories.md` in the same task or an explicitly coupled follow-up. Treat registration as incomplete until that sync is done, unless the human explicitly decides the project must stay omitted from profile documentation.
