...
- Adding a project to `workspace.yaml` requires a coordinated `syllik/syllik` change for the `README.md` project navigation, `docs/workspace.md`, and `docs/repositories.md`. The registry change is not ready for human merge until that profile-documentation change exists and is synchronized with it. Omission from profile documentation requires an explicit human decision.
