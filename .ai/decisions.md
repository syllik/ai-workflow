...
- Git lifecycle is explicit per workspace record: `lifecycle` declares deployment profile and `branchState` declares canonical, migration, external, or approved integration-exception branch semantics. Managed canonical history converges on `master`; staging/production are deployment environments rather than mandatory branches.
