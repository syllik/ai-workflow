...
5. Создайте или обновите связанный change в `syllik/syllik`: project navigation в `README.md`, `docs/workspace.md` и `docs/repositories.md`. PR с изменением `workspace.yaml` не считается готовым к human merge, пока синхронизирующий profile-documentation change не существует и не соответствует ему; исключение требует явного решения человека не публиковать проект в profile documentation.
